...
<div class="row">
	<form action="/scarch_action" method="get">
		...
		<label for="exampleInputBorder">Name</label>
			<input type="text" class="" name="search" placeholder="Enter name" required>
		<button type="submit" class="btn btn-info btn-sm">Save</button>
	</form>

@foreach ($search2 as $search2)
...
<div class="col-md-3">
	<figure class="card card-product">
		...
		<div class="img-wrap"> 
			<img src="{{asset('pic/'.$search2->product_photo)}}">
			<a class="btn-overlay" href="#"><i class="fa fa-search-plus"></i> Quick view</a>
		</div>
		<figcaption class="info-wrap">
			<a href="#" class="title">{{Str::limit($search2->product_name, 10)}}</a>
            
			<div class="action-wrap">
                <form action="/storaddcart/{{$search2->id}}" method="post">
                    @csrf
					
					<input type="hidden" class="form-control" name="product_name" value={{$search2->product_name}}>
					<input type="hidden" class="form-control" name="product_quantity" value={{$search2->product_price}}>
					<input type="hidden" class="form-control" name="product_price" value={{$search2->product_price}}>
				<button type="submit" name="product_id" class="btn btn-primary btn-sm float-right"> <i class="fa fa-cart-plus"></i> Add </button>
                
            </form>
               
				<div class="price-wrap h5">
					<span class="price-new">${{$search2->product_price}}</span>
				</div> <!-- price-wrap.// -->
			</div> <!-- action-wrap -->
		</figcaption>
	</figure> <!-- card // -->
...
<td>
<figure class="media">
<div class="img-wrap"><img src="{{asset('pic')}}/{{App\Models\product::find($addcart->product_id)->product_photo}}" class="img-thumbnail img-xs"></div>

<figcaption class="media-body">
    <h6 class="title text-truncate"> {{Str::limit(App\Models\product::find($addcart->product_id)->product_name, 10)}}</h6>
</figcaption>
</figure> 
</td>
...
<td> 
    <div class="price-wrap"> 
        <var class="price">${{App\Models\product::find($addcart->product_id)->product_price}}</var> 
    </div> <!-- price-wrap .// -->
</td>
<td class="text-right"> 
...
<div class="col-md-6">
    ...
    <a href="/viewinvoice" class="btn  btn-primary btn-lg btn-block"><i class="fa fa-shopping-bag"></i> Charge </a>
     </form>
</div>
<br>
<br>
...
<center>  
    ...
    <a href="/viewinvoice" <i class="fas fa-print"></i> Print invoice</a>
