            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <table id="datatablesSimple">
                        ...
                        <tbody>
                            @foreach ($dataff as $key=>$dataff)
                            <tr>
                                <td>{{ $key+1}}</td>
                                
                                <td>
                                    
                                    {{Str::limit(App\Models\product::find($dataff->product_id)->product_name, 10)}}
                                </td>
                                <td>{{Str::limit(App\Models\product::find($dataff->product_id)->catagory, 10)}}</td>
                                <td>{{ $dataff->quantity}}</td>
                                <td>${{ $dataff->product_price}}</td>
                                <td>
                                   
                                    <a href="/delete_order/{{ $dataff->id }}" class="btn btn-danger" role="button" aria-pressed="true">Delete</a>
                                </td>
                            </tr>
